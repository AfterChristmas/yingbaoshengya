 ...
 * @Auther: wanyan
 */
public class TestKnowPointsAty extends BaseActivity implements TreeNode.TreeNodeClickListener {
            ...
            protected void onSuccees(List<VideoKnowPointResult.ResultBean> resultBeans)  {
                ...
                if (pid!=0){
                    for (int i = 0; i < resultBeans.size(); i++) {
                        resultBeans.get(i).setHasChild(0);
                    }
                }
    ...
    public void onClick(TreeNode node, Object value) {
        ...
        if (!node.isExpanded() && treeNode.getChildren().isEmpty() && item.knowledgePointBean.getHasChild()== 1) {
            ...
            Intent  intent = new Intent(TestKnowPointsAty.this, KnowledgeTestActivity.class);
            intent.putExtra("subjectId", subjectId);
            intent.putExtra("kpId", item.knowledgePointBean.getId());
