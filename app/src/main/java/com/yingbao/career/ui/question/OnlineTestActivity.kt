...
class OnlineTestActivity : BaseActivity(),View.OnClickListener {
    ...
    private fun loadQuestion(itemPosition:Int) {
        ...
        map["stageId"] = 4.toString()
        map["subjectId"] = subjectId.toString()
        map["num"] = "5"
        RetrofitFactory.getInstance().getRandomQuestion(map,object :BaseObserver<List<RandomQuestionBean.ResultBean>>() {
                        ...
                        for (questionEntity in list) {
                            if (questionEntity.questionDetails.size == 1) {
                               val detailTemp =  questionEntity.questionDetails.get(0);
                                ...
                                quesDetail.categoryId = subjectId
