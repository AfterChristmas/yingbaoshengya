package com.yingbao.career.video.database;
...
import com.yingbao.career.video.PolyvDownloadInfo;
...
public class PolyvDownloadSQLiteHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL(
                "create table if not exists  downloadlist(vid varchar(20),title varchar(100),duration varchar(20),filesize int,bitrate int,fileType int,percent int default 0,total int default 0,courseid int default 0,videocover varchar(100),primary key (vid, bitrate, fileType))");
    }
    ...
    public void insert(final PolyvDownloadInfo info) {
        SINGLE_EXECUTOR.execute(new Runnable() {
            ...
            public void run() {
                ...
                String sql = "insert into downloadlist(vid,title,duration,filesize,bitrate, fileType,courseid,videocover) values(?,?,?,?,?,?,?,?)";
                db.execSQL(sql, new Object[]{info.getVid(), info.getTitle(), info.getDuration(),
                        info.getFilesize(), info.getBitrate(), info.getFileType(),info.getCourseId(),info.getVideoCover()});
            }
        });
    }
    ...
    private ContentValues createValues(PolyvDownloadInfo info) {
        ...
        contentValues.put("courseid", info.getCourseId());
        contentValues.put("videocover", info.getVideoCover());
    ...
    public LinkedList<PolyvDownloadInfo> getAll() {
        ...
        SQLiteDatabase db = getWritableDatabase();
        String sql = "select vid,title,duration,filesize,bitrate,fileType,percent,total ,courseid ,videocover from downloadlist";
        Cursor cursor = null;
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                String videocover = cursor.getString(cursor.getColumnIndex("videocover"));
                PolyvDownloadInfo info = new PolyvDownloadInfo(vid, duration, filesize, bitrate, title,courseId,videocover);
     ...
     * @return
     */
    public LinkedList<PolyvDownloadInfo> getDownloadInfoById(String queryId) {
        LinkedList<PolyvDownloadInfo> infos = new LinkedList<PolyvDownloadInfo>();
        SQLiteDatabase db = getWritableDatabase();
        String sql = "select vid,title,duration,filesize,bitrate,fileType,percent,total,courseid,videocover from downloadlist  where vid=?";
        Cursor cursor = null;
        try {
            cursor = db.rawQuery(sql, new String[]{queryId});
            while (cursor.moveToNext()) {
                String vid = cursor.getString(cursor.getColumnIndex("vid"));
                String title = cursor.getString(cursor.getColumnIndex("title"));
                String duration = cursor.getString(cursor.getColumnIndex("duration"));
